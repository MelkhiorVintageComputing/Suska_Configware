...
--   Udo Matthe fix reading the outputs when port direction is output.
--
...
entity WF2149IP_DIGOUT_TOP_SOC is
    port(
...
end WF2149IP_DIGOUT_TOP_SOC;

architecture STRUCTURE of WF2149IP_DIGOUT_TOP_SOC is
signal BUSCYCLE     : BUSCYCLES;
signal BDIR_BC      : std_logic_vector(2 downto 0);
...
begin
    ...
    I_PSG_WAVE: WF2149IP_WAVE_DIGOUT
        port map(
            RESETn      => RESETn,
            SYS_CLK => SYS_CLK,
    ...
    DA_OUT <=   DATA_OUT_I when DATA_EN_I = '1' else -- WAV stuff.
                PORT_A when BUSCYCLE = R_READ and ADR_I = x"E" and CTRL_REG(6) = '1' else
                PORT_B when BUSCYCLE = R_READ and ADR_I = x"F" and CTRL_REG(7) = '1' else
