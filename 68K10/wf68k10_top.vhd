...
--   Correction in the 68K00 mode bus error stack frame (see TMP_CPY).
...
--   Top level: Fixed the ALU_OP2_IN multiplexer for the CHK instruction.
...
--   Opcode decoder: removed breakpoint insert logic.
...
entity WF68K10_TOP is
    generic(
        VERSION         : std_logic_vector(31 downto 0) := x"20210815"; -- CPU version number.
        ...
        NO_INDEXSCALING : boolean := true); -- Supress effective address calculation with scaled index. Set true for compatibility reasons.
...
signal FC_CPY                   : std_logic_vector(2 downto 0);
...
signal OPC_ACCESS_CPY           : std_logic;
...
begin
    ...
    begin
        ...
        if BUSY_MAIN = '1' then
            ...
            FC_CPY <= FC_OUT_I;
            INSTRUCTIONn <= not OPC_ACCESS_CPY;
        end if;
    end process TMP_CPY;
    ...
    DATA_EXH <= SR_CPY & PC(31 downto 16) when K6800n = '0' and STACK_FORMAT = x"0" and STACK_POS = 2 else -- 68K00 short stack frame.
                ...
                x"00" & "000" & RWn_CPY & INSTRUCTIONn & FC_CPY & FAULT_ADR(31 downto 16) when K6800n = '0' and STACK_FORMAT = x"8" and STACK_POS = 2 else -- 68K00 long stack frame.
    ...
    -- The BITPOS is valid for BCHG, BCLR, BSET and BTST. It spans 0 to 31 bytes when
    -- it is in register direct mode. It is modulo 8 in memory manipulation mode.
    BITPOS <= BIW_1(4 downto 0) when (OP = BCHG or OP = BCLR or OP = BSET or OP = BTST) and BIW_0(8) = '0' and ADR_MODE = "000" else
              ...
              DR_OUT_1(4 downto 0) when (OP = BCHG or OP = BCLR or OP = BSET or OP = BTST) and ADR_MODE = "000" else "00" & DR_OUT_1(2 downto 0);
