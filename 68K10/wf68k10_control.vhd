...
begin
    ...
    LOAD_OP3 <= '1' when (OP = DIVS or OP = DIVU) and OP_SIZE_I = LONG and INIT_ENTRY = '1' else '0'; -- 64 bit operand.
    ...
    TRAP_ILLEGAL <= '1' when OP = BKPT and FETCH_STATE = FETCH_OPERAND and RD_RDY = '1' else '0';
    ...
    EX_TRACE <= '0' when OP = BKPT or OP = ILLEGAL or OP = UNIMPLEMENTED else
                '1' when TRACE_EN = '1' and OPD_ACK = '1' and FETCH_STATE = START_OP and OP = TRAP else -- Remark: no tracing for RTE.
    ...
    FETCH_DEC: process(ADR_MODE_I, ALU_BSY, AR_IN_USE, BIW_0, BIW_1, BRANCH_ATN, DR_IN_USE, EW_ACK, 
    ...
    begin
        case FETCH_STATE is
            when START_OP =>
                ...
                --elsif ALU_BSY = '1' or EXEC_WB_STATE /= IDLE then
                --    NEXT_FETCH_STATE <= START_OP;
                else
                    case OP is
