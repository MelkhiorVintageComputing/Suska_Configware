...
--   Minor changes to meet Falcon core requirements.
--   Blitter is 32 address bus capable now.
--
...
port (  CLK         : in std_logic;
        ...
        -- The flags:
        FXSR    : buffer std_logic;
        NFSR    : buffer std_logic;
        OP      : buffer std_logic_vector(3 downto 0);
        HOP     : buffer std_logic_vector(1 downto 0);
        HOG     : buffer std_logic;
        BUSY    : buffer std_logic;
        SMUDGE  : buffer std_logic
      );
end BLITTER_CORE;

architecture BEHAVIOR of BLITTER_CORE is
-- System signals:
...
begin
        ...
        if RESET = '1' then
            ...
            if  WRITEWORD = true and SUPERUSER = true then
                case ADR_I is
                    ...
                    when x"FFFF8A24"    => SRC_ADR(31 downto 16) <= DATA_IN;
                    ...
                    when x"FFFF8A32"    => DEST_ADR(31 downto 16) <= DATA_IN;
    ...
    DATA_OUT <= HALFTONE_RAM(0) when ADR_I = x"FFFF8A00" and READWORD = true else
                ...
                "000000" & HOP & x"0" & OP when ADR_I = x"FFFF8A3A" and READWORD = true else
                BUSY & HOG & SMUDGE & '0' & LINENUMBER & FXSR & NFSR & "00" & SKEW when ADR_I = x"FFFF8A3C" and READWORD = true else
                -- Byte wide access:
                "000000" & HOP & x"00" when ADR_I = x"FFFF8A3A" and READHI = true else  -- x"FF8A3A".
                x"000" & OP when ADR_I = x"FFFF8A3A" and READLOW = true else -- x"FF8A3B".
                BUSY & HOG & SMUDGE & '0' & LINENUMBER & x"00" when ADR_I = x"FFFF8A3C" and READHI = true else -- x"FF8A3C".
                x"00" & FXSR & NFSR & "00" & SKEW when ADR_I = x"FFFF8A3C" and READLOW = true else -- x"FF8A3D".
    ...
    P_SRC_BUFF: process(CLK, SKEW, SRC_BUFF)
    begin
        if CLK = '0' and CLK' event then
            if RESET = '1' then
                SRC_BUFF <= (others => '0');
            elsif FETCHSRC = '1' and SRC_X_INCR(15) = '0' then -- Positive SRC_X_INCR value.
        ...
        case SKEW is
            ...
            when others => HOP_SRC <= SRC_BUFF(30 downto 15); -- x"F".
        end case;
    end process P_SRC_BUFF;

    -- Halftone source data processing:
    HALFTONE <= HALFTONE_RAM(To_Integer(unsigned(LINENUMBER))) when SMUDGE = '0' else -- Line number addressing.
